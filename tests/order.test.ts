...
// The cart-clearing observer calls getCartItemRepository().delete(...), so the
// mocked repository has to hand back something with a delete method.
...
describe('Orders API', () => {
    ...
    it('TC-031: completes checkout, charges tax and returns a processed order', async () => {
        ...
        // Only the last four digits of the card are ever persisted.
    ...
    it('TC-034: declines the third consecutive payment request and denies the order', async () => {
        // The simulator fails every third request, so two successful checkouts
        // are run first to reach the failing one.
        ...
        // The order is still written, marked denied, so the attempt is auditable.
    ...
    it('TC-035: an approved order notifies the catalog, analytics and cart observers', async () => {
        ...
        // Catalog observer: stock reduced by the quantity bought.
        expect(mockedOrderRepo.decrementVehicleQuantity).toHaveBeenCalledWith(
            VEHICLE_ID,
            1
        );

        // Analytics observer: the sale recorded for the administrator reports.
        ...
        // Cart observer: the cart that produced the order emptied.
