...
/**
 * Reduce a vehicle's stock and mark it sold out when nothing is left.
 *
 * The decrement is written as a single conditional UPDATE rather than a
 * read-then-write, so two customers checking out the last unit at the same time
 * cannot both succeed. The affected row count tells the caller which one won.
 */
